...
Census_2023_Finished  %>% 
  mutate(ExactDate = mdy_hms(date_measured, tz = Sys.timezone()))  %>% 
  pull(date_measured,ExactDate)  %>% 
  head()
...
### 2018 to 2023 ###
    ...
    Census_2018_Simplified <- Census_2018_Finished[c("UID","quadrat" ,"dbh", "status","Genus", "Species", "Calculated_ABG_2018")]
    ...
    Woody_growth_2023 <- Woody_growth_18_23  %>% 
      mutate(Woody_Growth_1823 = Calculated_ABG_2023 - Calculated_ABG_2018,
           quadrat = sprintf("%04d",as.numeric(quadrat)))  %>% 
      group_by(quadrat)  %>% 
      summarize(Abg_C_Mg_yr = sum(Woody_Growth_1823,na.rm = T)/1000 / 5 * .47)  %>% 
      mutate(Year = 2023)
