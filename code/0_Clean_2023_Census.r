...
census2023_checks <- census2023  %>% 
    ...
    mutate(EditDate = as.Date(cut(as.POSIXct(EditDate, format = "%m/%d/%Y %I:%M:%S %p"), "day")),
           ...
           dateflag = if_else((MeasureDate > EditDate) | is.na(MeasureDate),T,F),
           textflag = if_else(grepl(pattern = ptrn,tolower(notes_current)),T,F),
           homflag = if_else(hom %in% c(0) & status_current %in% c("LI"), T, F))  %>% 
    group_by(tag)  %>% 
    arrange(tag,StemTag)  %>%  
...
quadrat_modal_date_measurement <- census2023_checks  %>% 
    filter(!dateflag)  %>% 
...
census2023_corrections <- census2023_checks  %>%  
    mutate( true_sp = if_else(textflag,str_extract(tolower(notes_current),paste0("(is) (",paste(scbi.spptable$sp, collapse = "|"),")"), group = 2),NA),
...
scbi.stem4.corrected <- xx ###make columns 
